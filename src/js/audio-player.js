 ...
 * Handles base64 MP3 chunks from active TTS provider and plays them seamlessly.
 */
...
class AudioPlayer {
    ...
    _scheduleNext() {
        ...
        source.connect(this._gainNode || this.audioContext.destination);
    ...
    setVolume(volume) {
        ...
        const clamped = Number.isFinite(numeric) ? Math.min(1, Math.max(0, numeric)) : this._volume;
        this._volume = clamped;
        if (this.audioContext && this._gainNode) {
            this._gainNode.gain.setValueAtTime(clamped, this.audioContext.currentTime);
        }
    }
